...
public class MessageReceiveEventHandler extends MessageReceiveEvent {

    private static String lastPlayerName;

    public MessageReceiveEventHandler(String formatted, String unFormatted) {
        super(formatted, unFormatted);
    }
    ...
    public Boolean call() {
        ...
        if (StatsAddon.getInstance().getCurrentGamemode() != null && StatsAddon.getInstance().getStatsChecker().getQueue().size() > 0) {
            if (unFormatted.startsWith(" ") || unFormatted.startsWith("-=")) {
                new Thread(() -> {
                    ...
                    if (unFormatted.toLowerCase().startsWith("-=")) {
